...
class RuntimeHWConfig:
    ...
    def get_boot_simulation_command(self, macaddr, blkdev, slotid,
        ...
        basecommand = """screen -S fsim{slotid} -d -m bash -c "script -f -c 'stty intr ^] && sudo ./{driver} +permissive $(sed \':a;N;$!ba;s/\\n/ /g\' {runtimeconf}) +macaddr={macaddr} +blkdev={blkdev} +slotid={slotid} +niclog=niclog {tracefile} +trace-start0={trace_start} +trace-end0={trace_end} +linklatency={linklatency} +netbw={netbw} +profile-interval=-1 +profile-interval={profile_interval} +zero-out-dram +permissive-off {bootbin} && stty intr ^c' uartlog"; sleep 1""".format(
                ...
                bootbin=bootbin, tracefile=tracefile,
